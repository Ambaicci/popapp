...
function SearchResultsScreen({ query, onClose, onLike, onComment, onShare, onCreatorClick, formatLikes }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, top: '120px', bottom: '70px', overflowY: 'auto', padding: '0 14px', background: '#090909', zIndex: 10 }}>
      ...
      {/* Tabs */}
      <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', borderBottom: '0.5px solid #222', paddingBottom: '10px' }}>
        {['all', 'videos', 'creators', 'pop-movies', 'hashtags'].map(tab => (
          <button 
            key={tab}
            onClick={() => setActiveTab(tab)}
            style={{ 
              background: activeTab === tab ? '#FF4F2E' : 'transparent',
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {results.creators.slice(0, activeTab === 'all' ? 3 : results.creators.length).map(creator => (
              <div key={creator.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px', background: '#111', borderRadius: '12px', cursor: 'pointer' }} onClick={() => onCreatorClick(creator.id, creator.username)}>
                <div style={{ width: '50px', height: '50px', borderRadius: '50%', background: '#FF4F2E', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 'bold' }}>
                  {creator.username?.charAt(0).toUpperCase()}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 'bold', color: 'white' }}>@{creator.username}</div>
                  <div style={{ fontSize: '11px', color: '#888' }}>{creator.video_count || 0} videos</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {(activeTab === 'all' || activeTab === 'pop-movies') && results.popMovies?.length > 0 && (
        <div style={{ marginBottom: '24px' }}>
          ...
          {results.popMovies.slice(0, activeTab === 'all' ? 3 : results.popMovies.length).map(movie => (
            <VideoCard key={movie.id} video={movie} onLike={onLike} onComment={onComment} onShare={onShare} onCreatorClick={onCreatorClick} formatLikes={formatLikes} />
          ))}
        </div>
      )}
        ...
        <div style={{ marginBottom: '24px' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {results.hashtags.map((tag, idx) => (
              <div key={idx} style={{ background: '#222', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', color: '#FF4F2E', cursor: 'pointer' }}>
